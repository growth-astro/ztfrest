...
def run_on_event(channel_id, program_ids=[1,2], bypass=False,
    ...
    if len(list_names) == 0:
        message = []
        message.append(f"No candidates with jd > {start_day.jd} and jd < {end_day.jd} and score > {score_thresh}")
        print("\n".join(message))
...
if __name__ == "__main__":
    ...
    for ii in range(len(days)):
        outfile = os.path.join(outdir, '%d-%d.dat' % (days[ii]-3, days[ii]))
        if not os.path.isfile(outfile):
            run_on_event(channel, program_ids=program_ids, bypass=True,
                         only_caltech=cfg.onlycaltech,
                         no_plots=cfg.noplots,
                         start_day=Time(days[ii]-3, format='jd'),
                         end_day=Time(days[ii],format='jd'),
                         outfile=outfile)
        with open(outfile, 'r') as f:
            candidates = [line.rstrip() for line in f]
        print(Time(days[ii], format='jd').isot, len(candidates))
