...
def check_lightcurve_alerts(username, password, list_names,
    ...
    for info in r['data']:
        if info['objectId'] in old:
            continue
        # changed from original for one-offs
        if (info['candidate']['jdendhist'] - info['candidate']['jdstarthist']) > max_days or info['candidate']['ndethist'] > 1:
            old.append(info['objectId'])
        objectid_list.append(info['objectId'])
    for name in set(objectid_list):
        if name in old:
            continue
        else:
            idx = [i for i in np.arange(len(objectid_list)) if objectid_list[i] == name][0]
            clean_set[name] = r['data'][idx]
        ...
        q = {"query_type": "find",
             "query": {
                       ...
                       "filter": {
                                  ...
                                  ##'classifications.braai': {'$gt': 0.9},
                                  # Looking for a one-off
                                  #'candidate.ndethist': {'$eq': 1},
                                  'candidate.isdiffpos': {'$in': ["t", 1, "1"]},
...
{Time(jd, format='jd').iso} + {jd_gap:.1f} days.")
        ...
        for info in r['data']:    
            ...
            if info['objectId'] in stellar_list:
                continue
            if np.abs(info['candidate']['ssdistnr']) < 10 and np.abs(info['candidate']['ssdistnr']) >= 0:
                continue
            ...
            if (info['candidate']['jdendhist'] - info['candidate']['jdstarthist']) > max_days:
                old.append(info['objectId'])
            try:
                # NEW! Remove also non-stellar sources for oneoffs
        ...
        # NEW: pass the candidates only if there are >5 in the same field, or if they are way too many
        if len(set_objectId) < 5 or len(set_objectId) > 100:
            continue
...
will be updated with the results of your queries.") 
    ...
    print("Checking durations.....only single detections here")
...
with the specified criteria.")
        ...
        # NEW for oneoffs, clean_set has all the relevant info for the alers
        outfile = 'test_oneoff.csv'
        with open(outfile, 'w') as of:
            of.write("name, ra, dec, jd, xpos, ypos, magpsf, sigmapsf, fid, ssdistnr, ssnamenr, programid\n")
            for k in clean_set.keys():
                of.write(f"{clean_set[k]['objectId']}, {clean_set[k]['candidate']['ra']}, {clean_set[k]['candidate']['dec']}, {clean_set[k]['candidate']['jd']}, {clean_set[k]['candidate']['xpos']},  {clean_set[k]['candidate']['ypos']}, {clean_set[k]['candidate']['magpsf']}, {clean_set[k]['candidate']['sigmapsf']}, {clean_set[k]['candidate']['fid']}, {clean_set[k]['candidate']['ssdistnr']}, {clean_set[k]['candidate']['ssnamenr']}, {clean_set[k]['candidate']['programid']}\n")
...
and {date_end.iso}")
    ...
    if names_matched is None:
        print("No matches found with LS DR8!")
        exit()
