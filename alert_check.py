...
user,password = 'sanand','PrettyStr0ngPa$$w0rd'
...
def get_alerts_around(radec,r,drb=0.25):
...
#                  "filter": {"candidate.drb": {"$gt": 0.25}},
    ...
    return k.query(query=q)
    

def check_alerts(radec,r=25,n='ztfcand', names_plot = False):
    if radec == None:
        radec,_,_ = get_radec(n)
#         print(radec,n)
    r = get_alerts_around(radec,r)
...
#             print(data[coord_name][i]['objectId'])
    ...
    for name in id_names:
        radec_,jdstart_,ndet_ = get_radec(name)
...
def plot_science(ax1,name,s_min,s_max):
...
def alert_check_complete(nn,plots = True,work_output = './output/'):
    f,id_names,jd_start,dists,times,v,cords,dist_from_alert,n_det = check_alerts(None,r=15,n=nn)
    if len(id_names) == 0:
#         nodata.append(nn)
        index_check = 3
    ...
    if plots:
        ...
        plot_science(ax4,nn,[1.001,1.001],[1.001,1.001])
        if len(ndet_to_plot) > 1:
            plot_science(ax3,names_to_plot[1],[1.001,1.001],[1.001,1.001])
            ax3.set_title(str(int(dist_to_plot[1]))+' arcsec, ' + str(ndet_to_plot[1]) +' det')
        if len(ndet_to_plot) > 2: 
            plot_science(ax5,names_to_plot[2],[1.001,1.001],[1.001,1.001])
